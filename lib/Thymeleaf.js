...
function processNode(element, context) {
	...
	ATTRIBUTE_PROCESSORS.forEach(attributeProcessor => {
		let attribute = `${attributeProcessor.prefix}:${attributeProcessor.name}`;
		if (!element.hasAttribute(attribute)) {
			attribute = `data-${attributeProcessor.prefix}-${attributeProcessor.name}`;
			if (!element.hasAttribute(attribute)) {
				return;
			}
		}
 ...
 * 
 * TODO: Should this be a separate module, express-thymeleaf?
 * 
 * @param {String} filePath
 * @param {Object} options
 * @param {Function} callback
 */
function processFileExpress(filePath, options, callback) {
...
exports.processFileExpress = processFileExpress;
